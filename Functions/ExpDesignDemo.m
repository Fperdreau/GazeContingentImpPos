function  [const, test] = ExpDesignDemo(const)
...
test.ntesttrials = size(test.design,1);
...
test.design = sortrows(test.design,-3); % sort per window size
test.design(1:20,:) = test.design(randperm(length(test.design(1:20,1))),:);
trialord = 21:size(test.design,1);
trialord = trialord(randperm(length(trialord)));
test.design(21:end,:) = test.design(trialord,:);

test.bloStep = test.ntesttrials/(test.nblock);

%% Devide the design into separated blocks
in = 1;
en = test.bloStep;
