function Data= expTrials(p, const, test, el, Keys, design, ntrials)
% Gaze-contingent window experiment.
% E.g: Data = ExpTrials(opt, const, test, el, Keys, design, ntrials)
%
...
while 1 && trial<ntrials
    ...
    for trial=1:ntrials
        ...
        % If full view condition, then simply show the full test picture
        if ms == const.Exp.apert(test.nwindow)
            foveaimdata = imdata;
            peripheryimdata = imdata;
        end
        ...
        % Hide cursor and set application priority
        HideCursor;
        ...
        % If participant did respond within the time limit, we blank the
        % screen and ask him/her to give a response anyway
...
%% Nested functions
    ...
    function endTrial
        % This function implements trial finalization routine:
        % - close windows
    ...
    function MsgEL(msg)
        % Wrapper function to send message to the EyeLink (if not in dummy
        % mode)
    ...
    function fix = infixationWindow(mx, my, fixationWindow)
        % This function test whether provided coordinates (mx, my) fall 
        % within a particular spatial region defined by fixationWindow
        % @param int mx: horizontal eye coordinate (in pixels)
        % @param int my: vertical eye coordinate (in pixels)
        % @param 1-by-4 vector fixationWindow: [top-x, top-y, width, height]
        %
        % @return bool fix: eye in fixation area (1: true)
    ...
    function imdata = convImg(img, h, w, rot)
        % This function loads a stimulus picture, color-transforms it,
        % scales it and rotate it.
        %
        % @param int img: stimulus id
        % @param int h: desired stimulus height (in pixels)
        % @param int w: desired stimulus width (in pixels)
        % @param float rot: desired orientation angle (in degree)
        %
        % @return m-by-n matrix: stimulus
